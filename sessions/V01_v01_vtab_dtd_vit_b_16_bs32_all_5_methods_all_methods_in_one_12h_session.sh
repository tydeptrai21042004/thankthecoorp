...
if [[ ! -s "$WEIGHT_FILE" ]]; then
  WEIGHT_TMP="${WEIGHT_FILE}.part"; rm -f "$WEIGHT_TMP"
  curl -L --fail --retry 5 --retry-delay 5 --connect-timeout 30 'https://storage.googleapis.com/vit_models/imagenet21k+imagenet2012/ViT-B_16-224.npz' -o "$WEIGHT_TMP"
  mv "$WEIGHT_TMP" "$WEIGHT_FILE"
fi
